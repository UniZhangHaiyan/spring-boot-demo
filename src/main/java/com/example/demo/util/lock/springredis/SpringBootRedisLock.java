package com.example.demo.util.lock.springredis;
...
public class SpringBootRedisLock {
    ...
    public void lock(String key, Long waitTime, Long durationTime) {
        ...
        try {
            ...
            e.printStackTrace();
            throw new RuntimeException("get lock exception");
        }
    ...
    private Boolean tryLock(String key, Long durationTime) {
        ...
        if (oldExpireTime != null && System.currentTimeMillis() > oldExpireTime) {
            ...
            Long currentExpireTime = Long.parseLong(redisTemplate.opsForValue().getAndSet(key, lockExpireTime.toString()).toString());
